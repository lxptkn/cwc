...
export default function EditClassForm({ cookingClass }: EditClassFormProps) {
  ...
  useEffect(() => {
    if (cookingClass) {
      ...
      setFormData({
        ...
        highlights: cookingClass.highlights || '',
        additionalInformation: cookingClass.additionalInformation || '',
        maxStudents: cookingClass.maxStudents || 10,
        price: cookingClass.price || 0,
        image: cookingClass.image || '',
      })

      // Set image preview if class has an image
      if (cookingClass.image) {
        setImagePreview(cookingClass.image)
      }

      // Parse existing dynamic fields
      if (cookingClass.menu) {
        const menuItems = cookingClass.menu.split(',').map(item => item.trim()).filter(item => item)
        setDishes(menuItems.length > 0 ? menuItems : [''])
      }

      if (cookingClass.highlights) {
        const highlightItems = cookingClass.highlights.split(',').map(item => item.trim()).filter(item => item)
        setWhatToBring(highlightItems.length > 0 ? highlightItems : [''])
      }

      if (cookingClass.additionalInformation) {
        const providedItems = cookingClass.additionalInformation.split(',').map(item => item.trim()).filter(item => item)
        setProvided(providedItems.length > 0 ? providedItems : [''])
      }
    }
  }, [cookingClass])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!session?.user?.id) {
      setError('Please sign in to edit a class')
      return
    }
    ...
    try {
      ...
      // Redirect to the updated class page
      router.push(`/class/${cookingClass.id}`)
    } catch (err) {
    ...
    <div className="min-h-screen bg-warm-bg dark:bg-gray-800">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-4xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-warm-fg mb-2">Edit Class</h1>
            ...
            <div className="bg-warm-bg-alt rounded-lg p-6 border border-warm-border">
              ...
              <div className="mb-6">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Upload an image for your class (JPG, PNG, GIF, WebP up to 5MB)
                </p>
              </div>
